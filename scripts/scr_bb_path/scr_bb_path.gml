...
function bb_grid_build() {
    ...
    global.path_grid={x0:_x0,z0:_z0,w:_w,h:_h,cells:array_create(_w*_h,-1),xs:[],zs:[],neighbors:[],door:[],routes:[],mask:-1,searches:0};
    var _n=global.path_grid;
...
function bb_grid_step(_x,_z,_tx,_tz) {
    var _n=global.path_grid,_start=bb_grid_nearest(_x,_z),_goal=bb_grid_nearest(_tx,_tz);
    if (_start<0 || _goal<0) return [_x,_z];
    if (_start==_goal) {
        if (!bb_walls_point(_tx,_tz,.3) && bb_on_floor(_tx,_tz)) return [_tx,_tz];
    ...
    for (var _i=0; _i<array_length(_n.routes); _i++) {
        if (_n.routes[_i].goal==_goal) { _next=_n.routes[_i].next;break; }
    }
    if (is_undefined(_next)) {
        _n.searches+=1;
        var _count=array_length(_n.xs);
        _next=array_create(_count,-1);
        var _queue=array_create(_count,0),_head=0,_tail=1;
        _queue[0]=_goal;_next[_goal]=_goal;
        while (_head<_tail) {
            var _cur=_queue[_head++],_neighbors=_n.neighbors[_cur];
            for (var _i=0; _i<array_length(_neighbors); _i++) {
                var _nb=_neighbors[_i];
                if (_next[_nb]>=0) continue;
                var _door=_n.door[_nb];
                if (_door>=0 && (_mask & (1<<_door))!=0) continue;
                _next[_nb]=_cur;_queue[_tail++]=_nb;
            }
        }
        if (array_length(_n.routes)>=32) array_delete(_n.routes,0,1);
        array_push(_n.routes,{goal:_goal,next:_next});
    }
